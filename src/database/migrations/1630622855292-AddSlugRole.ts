...
export class AddSlugRole1630622855292 implements MigrationInterface {
  name = 'AddSlugRole1630622855292';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`appmenu\`.\`roles\` ADD \`slug\` varchar(255) NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`appmenu\`.\`roles\` DROP COLUMN \`slug\``,
    );
  }
}
