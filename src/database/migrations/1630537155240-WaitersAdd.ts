...
export class WaitersAdd1630537155240 implements MigrationInterface {
  name = 'WaitersAdd1630537155240';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`appmenu\`.\`waiters\` ADD \`active\` tinyint NOT NULL DEFAULT 1`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`appmenu\`.\`waiters\` DROP COLUMN \`active\``,
    );
  }
}
