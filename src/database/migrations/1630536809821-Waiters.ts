...
export class Waiters1630536809821 implements MigrationInterface {
  name = 'Waiters1630536809821';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TABLE \`appmenu\`.\`waiters\` (\`id\` char(36) NOT NULL, \`name\` varchar(255) NOT NULL, \`key_access\` varchar(255) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deleted_at\` timestamp(6) NULL, \`storeId\` char(36) NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
    );
    await queryRunner.query(
      `ALTER TABLE \`appmenu\`.\`customers\` ADD \`deleted_at\` timestamp(6) NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`appmenu\`.\`customers\` DROP COLUMN \`deleted_at\``,
    );
    await queryRunner.query(`DROP TABLE \`appmenu\`.\`waiters\``);
  }
}
